...
```text
ROLE: senior engineer implementing HubbleOps Phase 3. Read CLAUDE.md, docs/ARCHITECTURE.md §6 and the Wrapper section, dev/proposals.md P-006, dev/context.md.
...
DEFINITION OF DONE:
1. packs/google_ads/rules/{python,php,javascript,typescript}.yml: families (a) sinks, (b) version carriers, (c) request-string sinks; each rule has an id and a test file under rules/tests/ with must-match / must-not-match snippets run by `ast-grep test`. packs/_mock/rules/ has a minimal equivalent. This order follows Google Ads' actual client-library ecosystem (Python, Java, .NET, PHP, Ruby, Perl, plus Node via REST): Python/PHP/JS/TS first, Java/C# in a later phase, nothing else until a customer needs it (P-006) — the wire and text channels already cover every other language with zero rules (§2, §6.6).
...
7. Missing/incompatible ast-grep → TOOLING_MISSING; if scan is forced, every INSIDE file becomes FILE_UNSCANNED; never a silently smaller ledger. Every INSIDE file in a language with no rule set (step 1's scope) gets STRUCTURE_UNSUPPORTED evidence instead of being silently skipped by the structure observer; Exposure Map reports structural coverage per language (P-006).
...
EVIDENCE REQUIRED: `ast-grep test` output for all rule files; fixture pass; printed wrapper chain for the DI fixture with each hop and carried params; metamorphic test output; test_no_provider_leak output; a scan of a file in a language with no rule set showing STRUCTURE_UNSUPPORTED rather than silence.
NON-GOALS: dynamic capture, verification, repair, field validation.
TRAPS: (1) precision-first pruning; (2) AI filter closing UNKNOWNs; (3) depth-limiting to 2–3 hops; (4) per-language walkers; (5) validating GAQL fields inside observe/; (6) an unsupported language silently producing zero structural candidates instead of STRUCTURE_UNSUPPORTED.
