...
- Own the task end to end. Every defect, blocker or surprise met while doing it is fixed in this session, in its own commit, before "done" is reported. Never report a problem back and stop, never defer a fix to a later task, never hand the operator a choice — pick the option that makes the outcome true and record the choice in dev/context.md. The only stop is a frozen surface (core/schemas, verify/verdict.py, the Observer, ProviderPack and Memory protocols): write the proposal, then finish everything that does not depend on it. The outcome is made true by fixing the engine, never by weakening a test, forcing an UNKNOWN closed or adding a repo-specific rule; those make the number a lie and defeat the goal.
...
- Verify an environment fix against a live shell before spending a run on it. Three successive attempts to isolate the per-family install were each plausible and each wrong — a Windows `;` PATH handed to bash, then a PATH export that `bash -lc` discarded because a login shell rebuilds it, then a venv with no pip because `uv venv` does not seed one. Each cost a full run to discover and none needed one to check. Fourth form, found by the CI baseline job: `uv sync` seeds no pip either, and scip-python shells out to pip, so a scan on a fresh sync exits TOOLING_FAILED; `uv pip install pip` after sync, verified in a live container first.
