...
from hubbleops.app.cli import EXIT_FAILED, main
...
def test_pack_verify_is_offline_and_refuses_every_unbound_replacement(
    ...
    assert unbound, "with nothing unbound this test asserts a refusal that cannot happen"
    monkeypatch.setattr(socket, "socket", denied)
    monkeypatch.setattr(socket, "create_connection", denied)
    assert main(["pack", "verify", "google_ads"]) == EXIT_FAILED
    captured = capsys.readouterr()
    assert captured.out == ""
    assert f"{len(unbound)} documented replacements" in captured.err
    for subject in unbound:
        assert subject in captured.err, subject
